...
function requireRecord(value: unknown): Record<string, any> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('EdgeOne invalid response')
  return value as Record<string, any>
}
function requireField(record: Record<string, any>, field: string): void {
  if (record[field] === undefined || record[field] === null) throw new Error(`EdgeOne response missing ${field}`)
}
...
export const edgeoneZoneListResponseSchema = {
  parse: (v: unknown): Record<string, any> => {
    ...
    if (!Array.isArray(r.Zones)) throw new Error('EdgeOne invalid zone list response')
    return {
      ...r,
...
export const edgeoneAccelerationDomainListResponseSchema = {
  parse: (v: unknown): Record<string, any> => {
    const r = requireRecord(v)
    if (!Array.isArray(r.AccelerationDomains)) throw new Error('EdgeOne invalid domain list response')
    return {
      ...r,
