...
export class DnsPodRecordOps {
  ...
  async check(providerId: string, zone: string, record: DnsSyncRecord): Promise<Record<string, unknown>> {
    const subdomain = this.subdomainFromFqdn(record.name, zone)
  ...
  private buildPayload(record: DnsSyncRecord, subdomain: string, line: string): RecordCreateInput {
    return {
  ...
  private baseResult(record: DnsSyncRecord): Record<string, unknown> {
    return {
      type: record.type,
      name: record.name,
      value: record.value,
