...
export async function runBatchItems(
  ...
  for (const raw of job.items) {
    ...
    await jobs.patchItem(
      job.id,
      (row) => options.itemKey(row) === key,
      { status: 'running', message: options.runningMessage },
      { current, message: options.progressMessage },
    )

    try {
      const result = await options.execute(item, key)
      await jobs.patchItem(
        job.id,
        (row) => options.itemKey(row) === key,
        {
...
export function dedupeStrings(values: string[], normalize: (v: string) => string = (v) => v.trim().toLowerCase()): string[] {
  const out: string[] = []
