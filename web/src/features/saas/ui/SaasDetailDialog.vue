...
<template>
  ...
  >
    ...
    <div v-else-if="hostname" class="space-y-4" :class="loading && 'opacity-60'">
      ...
      <div v-if="needsOwnershipHelp && ownership.name" class="rounded-md border">
      ...
      <div v-for="(rec, index) in dcvDelegationRecords" :key="`dcv-${index}`" class="rounded-md border">
      ...
      <Collapsible v-if="acmeTempRecords.length" class="rounded-md border">
        <CollapsibleTrigger
          class="text-muted-foreground flex w-full cursor-pointer items-center px-3 py-2 text-left text-sm"
        >
