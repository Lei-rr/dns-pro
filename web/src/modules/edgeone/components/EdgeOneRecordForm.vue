...
const showHttpsPort = computed(() => ['FOLLOW', 'HTTPS'].includes(form.value.origin_protocol as string))

function submit() {
  const error = validateEdgeOneRecordForm(form.value, domainSuffix.value)
  if (error) {
    message.error(errorMessage(error, '表单校验失败'))
    return
  }
  const payload = { ...submitPayload.value, autoSync: !props.modelValue && form.value.autoSync && props.dnspodLinked }
  emit('save', payload)
}
</script>
