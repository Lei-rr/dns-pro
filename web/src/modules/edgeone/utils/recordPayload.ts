...
export function formStateToRecordPayload(form: Record<string, unknown>, zoneName: string) {
  const domainName = fullDomainName(form.prefix as string, zoneName)
  return {
    ...form,
...
export function prefixFromDomain(domain: string, zoneName: string) {
...
export function validateEdgeOneRecordForm(form: Record<string, unknown>, zoneName: string): string {
  fullDomainName(form.prefix as string, zoneName)
  if (!validPrefix(form.prefix as string)) return '加速域名前缀格式不正确'
  if (!validDomain(fullDomainName(form.prefix as string, zoneName))) return '加速域名格式不正确'
