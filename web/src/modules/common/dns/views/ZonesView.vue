...
const statusColumns = computed(() => {
  return ((providerHook.value.zoneStatusColumns as Array<{ title: string; key: string; width?: number }>) || defaultProviderHook.zoneStatusColumns).map((column) => ({
    title: column.title,
    key: column.key,
...
const statusDefinitions = computed(() => {
  return (providerHook.value.zoneStatusColumns as Array<{ title: string; key: string; width?: number; getStatus?: (r: Record<string, unknown>) => unknown }>) || defaultProviderHook.zoneStatusColumns
})
...
function zoneAvatar(zone: string): string { return (String(zone || '').match(/[a-z0-9]/i)?.[0] || '#').toUpperCase() }
function avatarColor(): string { return resolveProviderAvatarColor(currentProviderMeta.value as Provider) }
function statusDefinition(key: string) { return statusDefinitions.value.find((item) => item.key === key) || null }
function zoneStatus(record: Record<string, unknown>, column: { getStatus?: (r: Record<string, unknown>) => unknown } | null) {
  return (column?.getStatus || ((item: Record<string, unknown>) => item.status || item.access_status || item.dns_status))(record)
}
function statusColor(record: Record<string, unknown>, column: { getStatus?: (r: Record<string, unknown>) => unknown } | null): string {
  return (providerHook.value.zoneStatusColor as (s: unknown) => string)(zoneStatus(record, column))
}
function statusText(record: Record<string, unknown>, column: { getStatus?: (r: Record<string, unknown>) => unknown } | null): string {
...
function openAddZone() { addZoneName.value = ''; showAddZone.value = true }
function zoneRouteId(zone: Record<string, unknown>): string { return zone.name as string }
...
function askRemove(zone: Record<string, unknown>) {
  modal.confirm({ title: '删除域名托管', content: `确认从 ${providerName.value} 删除 ${zone.name}？这会删除服务商中的域名托管和解析记录，不会删除注册商里的域名。若 NS 仍指向该服务商，解析可能中断。`, okText: '删除', okType: 'danger', cancelText: '取消', onOk: () => remove(zone) })
}
