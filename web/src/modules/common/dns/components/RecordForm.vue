...
function validate(): string {
  ...
  if (type === 'A' && !validIpv4(value)) return 'A 记录值必须是有效 IPv4 地址'
  if (type === 'AAAA' && !validIpv6(value)) return 'AAAA 记录值必须是有效 IPv6 地址'
  if (['CNAME', 'NS'].includes(type) && !validHostname(value)) return `${type} 记录值必须是有效域名`
  if (type === 'MX') {
    if (!validHostname(value)) return 'MX 记录值必须是有效域名'
    const priority = Number(form.value.priority || 10)
...
function validRecordName(value: string) {
