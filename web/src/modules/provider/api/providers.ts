...
function presentDefinitions(definitions: Array<Record<string, unknown>>): ApiResponse<ProviderDefinitions> {
  ...
  return { data: { types: definitions as unknown as ProviderDefinitions['types'], labels } }
}

export const providerSettingsApi = {
  providers: async (): Promise<ApiResponse<Provider[]>> => {
    const response = (await http.get(endpoints.providers)) as ApiResponse<Provider[]>
    return { ...response, data: response.data.map(presentProvider) }
  },
  providerDefinitions: async (): Promise<ApiResponse<ProviderDefinitions>> => presentDefinitions(((await http.get(endpoints.providerDefinitions)) as ApiResponse<Array<Record<string, unknown>>>).data),
  ...
  updateProviderOrder: async (order: string[]): Promise<ApiResponse<Provider[]>> => {
    const response = (await http.put(endpoints.providerOrder, { order })) as ApiResponse<Provider[]>
    return { ...response, data: response.data.map(presentProvider) }
  },
}
