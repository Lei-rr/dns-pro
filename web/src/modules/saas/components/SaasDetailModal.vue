    ...
    <template v-if="hostnameRecord">
      ...
      <a-descriptions bordered size="small" :column="1" style="margin-bottom: 16px">
        ...
        <a-descriptions-item label="主机名状态">
          <a-tag :color="statusColor(hostnameRecord.status as string)">{{ statusLabel(hostnameRecord.status as string) }}</a-tag>
        </a-descriptions-item>
        <a-descriptions-item v-if="hostnameRecord.custom_origin_server" label="回源服务器">{{ hostnameRecord.custom_origin_server }}</a-descriptions-item>
        <a-descriptions-item v-if="sslSettings.min_tls_version" label="最低 TLS 版本">{{ minTlsLabel(sslSettings.min_tls_version as string) }}</a-descriptions-item>
        <a-descriptions-item v-if="sslRecord.issuer" label="证书颁发">{{ sslRecord.issuer }}</a-descriptions-item>
        <a-descriptions-item v-if="sslRecord.expires_on" label="证书到期">{{ formatDate(sslRecord.expires_on as string) }}</a-descriptions-item>
        <a-descriptions-item label="证书状态">
          <a-tag :color="statusColor(sslRecord.status as string)">{{ statusLabel(sslRecord.status as string) }}</a-tag>
        </a-descriptions-item>
      </a-descriptions>
...
const ownershipVerification = computed(() => (hostnameRecord.value.ownership_verification as Record<string, unknown>) || {})
...
const errorMessages = computed(() => {
  ...
  for (const e of top) {
    ...
    else if (e && typeof e === 'object') out.push((e as { message?: string; error?: string }).message || (e as { message?: string; error?: string }).error || JSON.stringify(e))
  }
...
const acmeTempRecords = computed(() => {
  ...
  return records
    .map((r) => {
      if (r?.txt_name && r?.txt_value) return { type: 'TXT', name: r.txt_name as string, value: r.txt_value as string, status: r.status as string | undefined }
      if (r?.http_url && r?.http_body) return { type: 'HTTP', name: r.http_url as string, value: r.http_body as string, status: r.status as string | undefined }
      return null
    })
